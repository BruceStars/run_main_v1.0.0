...
wx.getFriendCloudStorage({
  ...
  success: res => {
    ...
    res.data.forEach((item, index) => {
      let userInfon = {}
      userInfon.name = item.nickname
      userInfon.scroes = item.KVDataList[0].value
      userInfon.key = index + 1
...
function init() {
  ...
  rankWidth = stageWidth * 4 / 6;
  rankHeight = stageHeight * 4 / 6;
  ...
  startY = offsetY_rankToBorder + preOffsetY + 60;
 ...
 */
function drawButton() {
  context_drawImage(assets.next, nextButtonX, nextButtonY, buttonWidth, buttonHeight);
  context_drawImage( assets.last, lastButtonX, lastButtonY, buttonWidth, buttonHeight);
}
...
function drawByData(data, i) {
  ...
  //绘制头像 assets.icon
  context_drawImage(data.url, x + 10, startY + i * preOffsetY + (barHeight - (avatarSize*0.8)) / 2, avatarSize*0.8, avatarSize*0.8);
